...
SELECT * FROM "COURSE_BOOKING" WHERE user_id =
(SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io');
...
WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明')
AND status IN ('上課中','已使用堂數') GROUP BY user_id;
...
SELECT "USER".name AS 教練名稱, "COACH".experience_years AS 經驗年數, "SKILL".name AS 專長名稱 FROM "COACH"
...
SELECT "SKILL".name AS 專長名稱, count("COACH".id) AS coach_total FROM "SKILL"
...
COUNT("CREDIT_PURCHASE".id) AS 銷售數量 FROM "CREDIT_PACKAGE"
...
-- 顯示須包含以下欄位： 總營收
SELECT SUM("CREDIT_PURCHASE".price_paid) AS 總營收 FROM "CREDIT_PURCHASE"
WHERE EXTRACT(MONTH FROM "CREDIT_PURCHASE".purchase_at) = 11;
